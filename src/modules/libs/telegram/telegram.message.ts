...
export const MESSAGES = {
  welcome:
    `<b>👋 Добро пожаловать в Kitsunaana Bot!</b>\n\n` +
    `Чтобы получать уведомления, давайте свяжем ваш Telegram аккаунт с Kitsunaana.\n\n` +
    `Нажмите на кнопку ниже и перейдите в раздел <b>Уведомления</b>, чтобы завершить настройку.`,

  authSuccess: `🎉 Вы успешно авторизовались и Telegram аккаунт связан с Kitsunaana!\n\n`,
  ...
  resetPassword: (token: string, metadata: SessionMetadata) =>
    `<b>🔒 Сброс пароля</b>\n\n` +
    `Вы запросили сброс пароля для вашей учетной записи на платформе <b>Kitsunaana</b>.\n\n` +
    `Чтобы создать новый пароль, пожалуйста, перейдите по следующей ссылке:\n\n` +
    `<b><a href="https://kitsunaana.store/account/recovery/${token}">Сбросить пароль</a></b>\n\n` +
    ...
    `Спасибо за использование <b>Kitsunaana</b>! 🚀`,
  deactivate: (token: string, metadata: SessionMetadata) =>
    ...
    `Спасибо, что пользуетесь <b>Kitsunaana</b>!\n\n` +
    `С уважением,\n` +
    `Kitsunaana`,
  accountDeleted:
    `<b>⚠️ Ваш аккаунт был полностью удалён.</b>\n\n` +
    `Ваш аккаунт был полностью стерт из базы данных Kitsunaana. Все ваши данные и информация были удалены безвозвратно. ❌\n\n` +
    ...
    `С уважением,\n` +
    `Единственный член команды Kitsunaana`,
  enableTwoFactor:
    `🔐 Обеспечьте свою безопасность!\n\n` +
    `Включите двухфакторную аутентификацию в <a href="https://kitsunaana.store/dashboard/settings">настройках аккаунта</a>.`,
}
